...
function VP = VelocityPotential(NB,NT,AK,ELM,VN) % SOLRAD
% NB = no. of (actual) fielf points P = no. of segments on each section
NQ = NB+1; % Source points Q are placed at the end of each segemnts
NP = NB+2; % two additional P points are placed to eliminate the problem of "irregular frequency"
% NT = no. of 2D Transverse Sections (Strip)
%% Calculation starts
...
% Initialize with Complex pi in Diagonal only for P points of NB >> pi*delta_mn
...
for I = 1:NT
    for K = 1:NB+1
        [FS2_SR(K),FS2_HD(K)] = FS2(I,K,AK,ELM); % -2FS{n}
    end
    ...
    for J = 1:NB 
        if (I <= NB)
            ...
            Tn_SR = 0; % zero for 2 additional points
            Tn_HD = 0;
        end
        % start with complex pi (Diagonal pi, other zero) eq(6.25)
        Dmn_SR(I,J) = Dmn_SR(I,J) + Tn_SR + (FS2_SR(J+1) - FS2_SR(J)); % [(n+1) - (n)]
...
for I = 1:NT
    for K = 1:NB %
        ...
        [Sn_SR,Sn_HD] = Sn2(I,K,AK,ELM); % -2Sn(x,y)

        if (I <= NB)
            Ln_SR = LnSwayRoll(I,K);    % [Ln(x,y) - Ln(x,-y)]
